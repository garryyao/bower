...
// List of supported AMD vendors.
var VENDORS = ["requirejs"];
...
function plant(amdVendor, options, config) {
    ...
    project.getTree()
        ...
        .then(function (packages) {
            // The AMD packages definition file that it updates.
            var packageFile = path.resolve(process.cwd(), config[amdVendor + 'Config']),
                df = Q.defer();
            ...
            switch (amdVendor) {
                case 'requirejs':
                    if (!fs.statSync(packageFile).isFile()) {
                        logger.error("not found", "requirejs config file not found:" + packageFile);
                        break;
                    }

                    requirejs.tools.useLib(function (require) {
                        var rjsConfig = fs.readFileSync(packageFile).toString();
                        rjsConfig = require('transform').modifyConfig(rjsConfig, function (config) {
                            ...
                            if (config.packages) {
                                ...
                                mout.object.forOwn(packages, function (pkg) {
                                    ...
                                    if (pkg.missing || name === 'requirejs')
                                        return;

                                    // handle rjs config "baseUrl"
                                    if(config.baseUrl){
                                        loc = path.relative(config.baseUrl, pkg.location);
                                        if(loc !== pkg.name){
                                            pkg.location = loc;
                                        } else {
                                            delete pkg.location;
                                            // don't write the package if all that left is the package "name"
                                            if (Object.getOwnPropertyNames(pkg).length === 1)
                                                return;
                                        }
                                    }
                                    ...
                                    if (existing) {
                                        // Update only if location is not yet specified.
                                ...
                                dist = mout.object.filter(dist, function (pkg, name) {
                                    ...
                                    if (options.prune && !(name in packages)) {
                                        logger.info("pruned", "Pruned requirejs package: " + name + " that is not found in installed components.");
                                        delete dist[name];
                                    }
                        ...
                        fs.writeFileSync(packageFile, rjsConfig);
                        df.resolve([packages, packageFile]);
                    });
                    break;
                default :
                    df.resolve([packages]);
            ...
            return df.promise;
        }).spread(function (value, file) {
            if(file) logger.info("saved", 'Updated ' + amdVendor + ' packages definitions in:' + file);
...
plant.vendors = function (cfg) {
    var presence = [];
    VENDORS.forEach(function (vendor) {
        if(vendor + 'Config' in cfg)
           presence.push(vendor);
    });
